"""4-group KM: Surgery (Yes/No) × Chemotherapy (Yes/No)
...
# Define 4 groups — same color scheme as AFP fig for readability
groups = [
    ('No Surgery / No Chemo', (df['surgery_any']==0) & (df['chemotherapy']==0), '#7f8c8d', '-'),
    ('No Surgery / Chemo',    (df['surgery_any']==0) & (df['chemotherapy']==1), '#95a5a6', '--'),
    ('Surgery / No Chemo',    (df['surgery_any']==1) & (df['chemotherapy']==0), '#2980b9', '-'),
    ('Surgery / Chemo',       (df['surgery_any']==1) & (df['chemotherapy']==1), '#2ecc71', '--'),
]
...
df['group'] = 'S' + df['surgery_any'].astype(str) + '_C' + df['chemotherapy'].astype(str)
pval = mlt(df['surv_months'], df['group'], df['vital_dead']).p_value

ax.set_xlim(0, 60)
ax.set_xlabel('Months')
ax.set_ylabel('Overall Survival')
ax.set_title('Surgery × Chemotherapy', fontweight='bold', loc='left')
...
plt.close()
print(f'  ✓ {name} — 4-group Surgery×Chemo KM')
